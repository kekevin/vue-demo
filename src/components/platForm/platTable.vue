...
// 	states:{
// 		rect1:{
// 				type:'topId1',text:{text:'字段过滤'}, attr:{ x:264, y:174, width:100, height:50},
// 				props:{
// 					text:{value:'字段过滤'},assignee:{value:''},form:{value:''},desc:{value:''}
// 				}
// 		}
// 	},
// 	paths:{
...
//config util rect path props editors init

//myFLow.js KKKKKK
function jqInit(that) {
  var myflow = {};
  myflow.config = {
    editable: true,
    lineHeight: 15,
  ...
  myflow.rect = function(o, r, id) {
    ...
    _img = _r
      .image(
        myflow.config.basePath + _o.img.src,
    ...
    _name = _r
      .text(
        ...
        _o.attr.y + myflow.config.lineHeight / 2,
        _o.name.text
      )
      .hide()
      .attr(_o.name);
    _text = _r
      .text(
        _o.attr.x + _o.img.width + (_o.attr.width - _o.img.width) / 2,
        _o.attr.y +
          (_o.attr.height - myflow.config.lineHeight) / 2 +
    ...
    var dragMove = function(dx, dy) {
      // 拖动中
      if (!myflow.config.editable) return;
    ...
    var bdragStart = function(ox, oy, t) {
      console.log("bdragStart", ox, oy, t); //_bx有问题 kkkkkk 重新定义var _bx
      _bx = ox;
      _by = oy;
    };
    ...
    var clickHandler = function(e, src) {
      console.log("var clickHandler = function");
      if (!myflow.config.editable) return;

      if (src.getId() == _id) {
        $(_r).trigger("showprops", [_o.props, src]);
        // console.log("$(_r)", $(_r));
        // console.log("src.getId()", src.getId());
        // console.log("src", src);
        
        // console.log("src.toJson()", src.toJson());
        let oJson = src.toJson();
        let typeId = src.toJson_type();
        // console.log(typeId);

        //下面五个无效
        // console.log('src.attr()',src.attr())
        // console.log('src.getBBox()',src.getBBox())
        // console.log('src.remove()',src.remove())
        // console.log('src.restore()',src.restore())
        // console.log('src.text()',src.text())

        that.showTabContent(_id, typeId, oJson);
      } else {
        hideBox();
      }
    };
    ...
    var textchangeHandler = function(e, text, src) {
      console.log("var textchangeHandler = function");
      if (src.getId() == _id) {
        _text.attr({
          text: text
        });
    ...
    // 转化json_type字串
    this.toJson_type = function() {
      console.log("toJson_type,_o.type---", _o.type);
      return _o.type;
    };
    // 转化json_text字串
    this.toJson_text = function() {
      return _o.text;
    };
    // 转化json_attr字串
    this.toJson_attr = function() {
      return _o.attr;
    };
    // 转化json_props字串
    this.toJson_props = function() {
      return _o.props;
    };

    // 从数据中恢复图
    this.restore = function(data) {
      console.log("this.restore = function(data)");
      var obj = data;
    ...
    this.getId = function() {
      //kkkkkk
      console.log("getId from  myflow.rect,_id:---", _id);
      console.log(this);
      return _id;
    };
  ...
  myflow.path = function(o, r, from, to) {
        ...
        switch (_t) {
          ...
          case "small": // 移动小点时，转变为大点，增加俩个小点
            if (
              ...
              var lt = new dot(
                "small",
                myflow.util.center(_lt.pos(), _pos),
                _lt,
                _lt.right()
              );
              _lt.right(lt);
              ...
              var rt = new dot(
                "small",
                myflow.util.center(_rt.pos(), _pos),
                _rt.left(),
                _rt
              );
              _rt.left(rt);
    ...
    var removerectHandler = function(e, src) {
      console.log("var removerectHandler = function");
      if (!myflow.config.editable) return;
    ...
    var rectresizeHandler = function(e, src) {
      console.log("var rectresizeHandler = function");
      if (!myflow.config.editable) return;
    ...
    // 转化json数据
    this.toJson = function() {
      console.log("this.toJson = function() ");
      var data =
        "{from:'" +
        _from.getId() +
        "',to:'" +
    ...
    // 删除
    this.remove = function() {
      console.log(" this.remove = function");
      _dotList.remove();
      _path.remove();
      _arrow.remove();
      _text.remove();
    ...
    this.getId = function() {
      console.log("2-get from  myflow.path ");
      return _id;
    };
    this.text = function() {
      return _text.attr("text");
  ...
  myflow.editors = {
    textEditor: function() {
      ...
      console.log("myflow.editors 属性窗口初始化");
    }
  };
  ...
  myflow.init = function(c, o) {
    ...
    $(_r).bind("addrect", function(e, type, o, id) {
      ...
      var rect = new myflow.rect(
        $.extend(true, {}, myflow.config.tools.states[type], o),
        _r,
        id
      );
    ...
    if (myflow.config.editable) {
      ...
      $("#cclick").click(function() {
        ...
        console.log(myflow);
        let clickGetShowData = getShowData();
        console.log("clickGetShowData", clickGetShowData);
        console.log(this);
        console.log(that);
      });
    ...
    if (o.restore) {
      ...
      if (data.paths) {
        for (var k in data.paths) {
          var p = new myflow.path(
            $.extend(true, {}, myflow.config.tools.path, data.paths[k]),
            _r,
  ...
  $.extend(true, myflow.config.tools.states, {
    start: {
      ...
      text: { text: "开始" },
      img: {
        src: "myflow/img/48/start_event_empty.png",
        width: 48,
        height: 48
      },
      ...
      text: { text: "结束" },
      img: {
        src: "myflow/img/48/end_event_terminate.png",
        width: 48,
        height: 48
      },
      attr: { width: 50, heigth: 50 },
    ...
    decision: {
      ...
      text: { text: "判断" },
      img: {
        src: "myflow/img/48/gateway_exclusive.png",
        width: 48,
        height: 48
      },
      attr: { width: 50, heigth: 50 },
...
export default {
  methods: {
    ...
    // oTabId----rect的Id typeId：topId1
    showTabContent(oTabId, typeId, json) {
      if (typeId) {
        // let id = typeId.includes("leftId")
        //   ? typeId.replace("leftId", "")
        //   : typeId.replace("topId", "");
        // this.$store.commit("modTabContent", id);
        // this.$store.commit("modTargetBtnId", typeId);

        let tId = oTabId.replace("rect", "");
        let oJson = eval("(" + json + ")");
        console.log(oJson);
        console.log("json.type:---", oJson.type);

        let { attr, props, text, type } = oJson;
        let sourceJ = {
          tabId: tId,
          type: type,
          attr,
    ...
    add(p) {
      ...
      this.$store.commit("modTabVal", p.name);
    }
  },
  data() {
    return {
